<div class="dishes">
	<h1 id="top">ARDOISE</h1>
	<div class="addDishBtn">
		<a href="/admin/ardoise/ajout"><button>Ajouter un plat</button></a>
	</div>

	<div class="searchContainer">
		<form action="/admin/ardoise/search" class="searchBar" method="POST">
	...
	<div class="btnFilter">
		<a href="#1"><button>Entrées</button></a>
		<a href="#2"><button>Plats</button></a>
		<a href="#3"><button>Desserts</button></a>
	</div>
	...
	<?php
	for ($i = 1 ; $i <= 3 ; $i++) {
		if($_SERVER['REQUEST_URI'] == '/admin/ardoise/search'){
			$dishes = Special::getFilter($search, $i);
		} else {
	...
	?>
			...
			?>
				<div class="dishCard" id="dish<?= $dish->id ?>">
					<div class="dishHead">
						<img src=<?= ($dish->image == 2) ? "/public/assets/specials/" . strtolower(str_replace(' ', '', $dish->id)) . ".jpg" : '/public/assets/baseImg/dish.jpg' ?> alt="Photo de <?= $dish->title; ?>">
						<form method="POST" action="/admin/ardoise/edit/img/<?= $dish->id ?>" enctype="multipart/form-data" class="formDishImg">
							<label for="<?= $dish->id ?>">
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
									<!--! Font Awesome Pro 6.2.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
					...
					<div class="dishBody" <?= ($dish->active == 2) ? 'style="background-color:rgba(1, 136, 1, 0.1)"' : '' ?>>
						<form method="POST" action="/admin/ardoise/edit/active/<?= $dish->id ?>" class="formMenuActive">
							<?php
							if ($dish->active == 2) {
							?>
						...
						<a href="/admin/ardoise/edit/<?= $dish->id ?>">Modifier</a>
...
<div class="modale">
	<div class="modaleContent">
		...
		<div class="modaleBtn">
			...
			<a class="deleteSpecialLink" href="">Supprimer</a>
		</div>
	</div>
</div>
...
<script src="../../public/assets/js/confirmSpecialDelete.js"></script>
